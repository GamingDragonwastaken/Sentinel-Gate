...
from sentinelgate.security.inspector import InspectionResult, inspect_prompt
from sentinelgate.security.policies import Policy, PolicyCheckResult, check_prompt_against_policies
from sentinelgate.security.risk_scorer import process_prompt
...
class SecurityContractTests(unittest.TestCase):
    ...
    @patch("sentinelgate.security.risk_scorer.log_request", return_value="audit-1")
    @patch("sentinelgate.security.risk_scorer.check_prompt_against_policies", return_value=PolicyCheckResult(False))
    @patch("sentinelgate.security.risk_scorer.get_active_policies", return_value=[])
    @patch("sentinelgate.security.risk_scorer.inspect_prompt")
    def test_unavailable_ingress_inspection_blocks_before_model_call(self, inspect_prompt, _policies, _check, log_request):
        ...
        with patch("sentinelgate.security.risk_scorer.call_gemini_via_lobster") as model_call:
    ...
    @patch("sentinelgate.security.risk_scorer.log_request", return_value="audit-2")
    @patch("sentinelgate.security.risk_scorer.check_prompt_against_policies")
    @patch("sentinelgate.security.risk_scorer.get_active_policies", return_value=[Policy(name="No secrets", natural_language="Never disclose secrets")])
    @patch("sentinelgate.security.risk_scorer.inspect_prompt")
    ...
    @patch("sentinelgate.security.risk_scorer.log_request", return_value="audit-3")
    @patch("sentinelgate.security.risk_scorer.inspect_response")
    @patch("sentinelgate.security.risk_scorer.call_gemini_via_lobster", return_value="model output")
    @patch("sentinelgate.security.risk_scorer.check_prompt_against_policies", return_value=PolicyCheckResult(False))
    @patch("sentinelgate.security.risk_scorer.get_active_policies", return_value=[])
    @patch("sentinelgate.security.risk_scorer.inspect_prompt")
    ...
    @patch("sentinelgate.security.risk_scorer.log_request", return_value="audit-4")
    @patch("sentinelgate.security.risk_scorer.check_prompt_against_policies", return_value=PolicyCheckResult(False))
    @patch("sentinelgate.security.risk_scorer.get_active_policies", return_value=[])
    @patch("sentinelgate.security.risk_scorer.inspect_prompt")
    ...
    def test_policy_provider_error_is_indeterminate(self):
        with patch("sentinelgate.security.policies.call_gemini_json", side_effect=RuntimeError("offline")):
    ...
    def test_audit_schema_persists_policy_status(self):
        from sentinelgate.database.audit_db import get_recent_logs, init_db, log_request
    ...
    def test_offline_demo_mode_exercises_safe_and_attack_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                from sentinelgate.database.audit_db import init_db
                from sentinelgate.security.risk_scorer import process_prompt
